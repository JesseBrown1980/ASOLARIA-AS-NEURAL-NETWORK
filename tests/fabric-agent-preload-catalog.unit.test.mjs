...
test('preload catalog includes the laws, tools, roles, and cadence agents need', () => {
  ...
  assert.ok(LAWS.includes('8-byte-host-process-replaces-node-per-agent-only-where-proven'));
  ...
  assert.ok(TOOLS.some((t) => t.id === 'program-cube-ingestion-map' && t.gate.includes('blind_mutation-0')));
  assert.ok(TOOLS.some((t) => t.id === 'agent-cost-layer-boundary' && t.gate.includes('free_compute_claim-0')));
